...
export const MCP_TOOL_CONTRACTS: McpToolContract[] = [
  ...
  c({ name: "list_credentials", domain: "credentials", source: "src/mcp/tools/credentials/credential-tools.ts", requiredScopes: ["credentials:read"], risk: "read_only", outputSchema: "credentialList", note: "Lists credential metadata; optional type filter replaces the former list_credentials_by_type.", exampleInput: { page: 1, pageSize: 10, search: "", type: "OPENAI" } }),
  ...
  c({ name: "run_workflow_test", domain: "executions", source: "src/mcp/tools/executions/execution-runtime-tools.ts", profiles: chatgptProfiles, requiredScopes: ["workflows:execute"], risk: "external_side_effect", outputSchema: "executionDetail", note: "Runs a workflow with curated sample data for manual/Google Form/Stripe triggers (consolidated from deprecated test_webhook_setup).", requiresApproval: true, externalSideEffect: true, exampleInput: { workflowId: "workflow-id", trigger: "google_form", approved: true } }),
  ...
  c({ name: "move_workflow_node", domain: "workflows", source: "src/mcp/tools/workflows/workflow-versioning.tool.ts", requiredScopes: ["workflows:write"], risk: "draft_write", outputSchema: "genericObject", note: "Moves a workflow node to a new canvas position. Cosmetic — no approval or version snapshot (Phase 1).", exampleInput: { workflowId: "workflow-id", nodeId: "node-id", position: { x: 0, y: 0 } } }),
