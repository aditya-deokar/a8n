...
> **Note:** Due to company policy, the actual source code and directory structure cannot be shared. The following describes the architectural layout conceptually.

The MCP module was organized into clearly separated concerns:

| Directory | Responsibility |
|---|---|
| `auth/` | API key service, bearer auth middleware, scope definitions (reused from Express), auth types |
| `middleware/` | Scope guard, Redis-backed rate limiter, structured audit logger, error boundary |
| `shared/` | Output sanitization and redaction, pagination helpers, Zod validation utilities |
| `tools/` | 11 domain folders (products, orders, shipments, inventory, SEO, pricing, customers, reports, categories, coupons, system) with a central registry |
| `resources/` | Business context resources (markdown content for AI consumption) |
...
> **Note:** Due to company policy, actual implementation code cannot be shared.

The central registry function registered tools from each of the 11 business domains. Each domain had its own registrar function that grouped related tools together. This made it straightforward to add new tool domains without modifying the core server logic.
...
> **Note:** Due to company policy, actual implementation code cannot be shared. The following describes the architectural pattern used across all 60+ tools.

Every tool followed a consistent structure with these steps:

1. **Extract authentication** from the incoming request context.
2. **Enforce scope** using the same permission scopes built into the Express APIs.
3. **Create audit context** for structured logging of the tool invocation.
4. **Validate inputs** using Zod schemas before any business logic executes.
5. **Call the business service** — the same service layer used by the admin dashboard, ensuring identical business rules and tenant isolation.
6. **Sanitize the response** by redacting any sensitive values before returning.
7. **Log the audit entry** with success or failure status, duration, and sanitized inputs.
...
> **Note:** Due to company policy, actual implementation code cannot be shared. The following describes the confirmation pattern.

When a high-risk tool was invoked without a confirmation hash, the handler returned a `confirmation_required` status with a warning message instead of executing the operation. Only after the AI client relayed the warning to the user and received explicit confirmation — passing back a valid confirmation hash — would the handler proceed with the destructive action. The confirmation hash was generated server-side and validated before execution, preventing replay or forged confirmations.
...
> **Note:** Due to company policy, actual implementation code cannot be shared. The following describes the eval case structure conceptually.

Each evaluation case contained a stable identifier for regression tracking, a business domain (products, orders, shipments, etc.), a user persona (product manager, order manager, admin), and a natural language business request as the goal. The expected outcomes included the list of tools that should be called, the required scopes, the business rules that must be enforced, the external side effects that should be triggered, and critically, the tools that should NOT be called — ensuring the AI did not over-reach.
...
Every MCP tool invocation generated a structured audit entry containing:

> **Note:** Due to company policy, actual data schemas cannot be shared.
...
| Tenant ID | The tenant context for isolation verification |
| Auth method | API key or session token |
| Duration | Execution time in milliseconds |
| Status | Success or failure |
| Sanitized input | Request parameters with secrets redacted |
| Scopes | The permission scopes used |
| Rate limit state | Remaining requests and current limit |
